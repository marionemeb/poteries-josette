...
- [~] Automatiser le déploiement vers OVH — `.github/workflows/deploy.yml` ajouté le 18/09/2026, déclenchement **manuel uniquement** (`workflow_dispatch`, décision volontaire vu qu'il s'agit du site de prod réel — pas de déploiement automatique à chaque push pour l'instant). Reproduit le process manuel actuel (FTP) : build assets (Node 12, requis pour `node-sass`, voir "Stack technique") + `composer install --no-dev`, puis synchronisation FTP en excluant `.git`, `.github`, `tests/`, `docker/`, `.env*`. Secrets GitHub configurés par l'utilisateur (`OVH_FTP_HOST`, `OVH_FTP_USERNAME`, `OVH_FTP_PASSWORD`, `OVH_FTP_REMOTE_DIR`, jamais vus par Claude).
  ...
  - **Toujours pas de déploiement réussi de bout en bout confirmé** au moment de cette note.
...
- [~] Mettre en place Dependabot — `.github/dependabot.yml` ajouté le 18/09/2026 : npm (devrait fonctionner, lockfile propre), composer (**expérimental** — à voir si Dependabot se heurte au même mur Composer 1/Packagist que nous, ou si son propre résolveur passe outre ; les futures PR de mise à jour composer, ou leur absence, donneront la réponse), github-actions (versions des actions utilisées dans les workflows). `cooldown: default-days: 21` sur les trois — délai de sécurité avant qu'une nouvelle version publiée soit proposée (cohérent avec la fonction shell `npm` de l'utilisateur, qui ajoute déjà `--before=21 jours` de son côté). Pas encore vérifié que le premier scan a tourné avec succès.
